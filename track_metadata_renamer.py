"""
================================================================================
Track Metadata Renamer
================================================================================
Author      : Breno Farias da Silva
Created     : 2026-08-15
Description :
    Core worker module for applying track metadata edits to Matroska video files.
    Reads reports generated by report.py and safely applies metadata changes using
    mkvpropedit. Handles video, audio, and subtitle track renaming with detailed
    error reporting and logging.

    Key features include:
        - Report-driven metadata modification workflow
        - Safe subprocess-based mkvpropedit execution
        - Granular error handling and recovery
        - Support for video, audio, and subtitle tracks
        - Detailed edit summaries and execution logs
        - Progress feedback during long operations

Usage:
    1. Import and call run_process_cli() from entry points
        from track_metadata_renamer import run_process_cli
        run_process_cli(input_directory, report_path)
    2. Or use worker functions directly for programmatic access
        from track_metadata_renamer import apply_track_metadata_edits
        apply_track_metadata_edits(file_path, edits, report_data)
    3. Generated reports automatically discovered in Reports/ directory

Outputs:
    - Modified Matroska files with updated track metadata
    - Logs/track_metadata_renamer.log (detailed operation log)
    - Summary report showing successful and failed edits
    - Terminal progress output with color-coded status

TODOs:
    - Implement automatic backup creation before edits
    - Add transaction-like semantics for batch operations
    - Support for edit rollback and recovery
    - Implement dry-run mode for preview before apply

Dependencies:
    - Python >= 3.8
    - report module (report structure and constants)
    - mkvpropedit_wrapper module (safe mkvpropedit interface)
    - Logger module (dual-channel console/file logging)
    - utils.utils module (timing and color utilities)

Assumptions & Notes:
    - Reports must exist and be recent for accuracy
    - Matroska files referenced in reports must still exist
    - mkvpropedit must be installed and available in PATH
    - File modifications are atomic at the file level
"""
